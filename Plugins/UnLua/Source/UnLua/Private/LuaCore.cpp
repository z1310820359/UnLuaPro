...
void* NewUserdataWithPadding(lua_State *L, int32 Size, const char *MetatableName, uint8 PaddingSize)
{
    if (Size < 1)
    {
        ...
        FString LogMsg = FString::Printf(TEXT("%s, Invalid size!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return nullptr;
    }
    if ((PaddingSize & 0x07) != 0)          // 8 bytes padding at least..., 8, 24, 88
    {
        // padding size must be greater or equal to 8 bytes
        FString LogMsg = FString::Printf(TEXT("%s, Invalid padding size!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return nullptr;
    }
    ...
    if (MetatableName)
    {
        ...
        if (!bSuccess)
        {
            FString LogMsg = FString::Printf(TEXT("%s, Invalid metatable, metatable name: %s!"), ANSI_TO_TCHAR(__FUNCTION__), UTF8_TO_TCHAR(MetatableName));
            UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
            return nullptr;
        }
    }
...
void* CacheScriptContainer(lua_State *L, void *Key, const FScriptContainerDesc &Desc)
{
    if (!Key)
    {
        FString LogMsg = FString::Printf(TEXT("%s, Invalid key!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return nullptr;
    }
...
void PushObjectCore(lua_State *L, UObjectBaseUtility *Object)
{
    ...
    if (!bSuccess)
    {
        FString LogMsg = FString::Printf(TEXT("%s, Invalid metatable,Name %s, Object %s,%p!"), ANSI_TO_TCHAR(__FUNCTION__), *MetatableName, *Object->GetName(), Object);
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
    }
}
...
static void PushPropertyArray(lua_State *L, T *Property, void *Value, void(*PushFunc)(lua_State*, T*, void*), const char *MetatableName = nullptr)
{
    ...
    {
        FString LogMsg = FString::Printf(TEXT("%s, Invalid parameters!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return;
    }
#endif
    ...
    {
        FString LogMsg = FString::Printf(TEXT("%s, Invalid metatable name!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return;
    }
...
bool GetObjectMapping(lua_State *L, UObjectBaseUtility *Object)
{
    if (!Object)
    {
        FString LogMsg = FString::Printf(TEXT("%s, Invalid object!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return false;
    }
...
int32 Class_CallLatentFunction(lua_State *L)
{
    ...
    {
        FString LogMsg = FString::Printf(TEXT("%s: Can't call latent action in main lua thread!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Warning, LogMsg);
        return 0;
    }
