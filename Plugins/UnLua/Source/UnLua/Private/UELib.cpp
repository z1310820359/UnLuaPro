...
static int32 Global_NewObject(lua_State *L)
{
    ...
    if (NumParams < 1)
    {
        ...
        UNLUA_LOGERROR(L, LogUnLua, Log, LogMsg);
        return 0;
    }
    ...
    if (!Class)
    {
        FString LogMsg = FString::Printf(TEXT("%s: Invalid class!"), ANSI_TO_TCHAR(__FUNCTION__));
        UNLUA_LOGERROR(L, LogUnLua, Log, LogMsg);
        return 0;
    }
...

    {
        ...
        {
            FString LogMsg = FString::Printf(TEXT("%s: Failed to new object for class %s!"), ANSI_TO_TCHAR(__FUNCTION__), *Class->GetName());
            UNLUA_LOGERROR(L, LogUnLua, Log, LogMsg);
            return 0;
        }
    }
