    ...
    for (int32 i = 0; i < Properties.Num(); ++i)
    {
        ...
        {   
            ...
            if (Property->CheckPropertyType(L, FirstParamIndex + ParamIndex, ErrorMsg))
                ...
                FString LogMsg = FString::Printf(TEXT("Invalid parameter type calling ufunction : %s,parameter : %d, error msg : %s"), *FuncName, ParamIndex, *ErrorMsg);
                UNLUA_LOGERROR(L, LogUnLua, Error, LogMsg);
            }
#else
            CleanupFlags[i] = Property->WriteValue_InContainer(L, Params, FirstParamIndex + ParamIndex, false);
#endif
...
#if ENABLE_TYPE_CHECK == 1
            FString LogMsg = FString::Printf(TEXT("FuncName %s has return value, but no value found on stack!"), *FuncName);
            UNLUA_LOGERROR(L, LogUnLua, Error, LogMsg);
#endif
        }
        else
